...
inline agsis::vect<double, D> cubic_spline_gradient(const ArrayDescriptor<T, D, RC> &grid, const agsis::vect<double, D> &p)
{
    ...
    for (int i = D - 1; i >= 0; i--)
    {
        ...
        i1d = static_cast<double>(index) -                           static_cast<double>(grid.strides[i]);

        i3d = static_cast<double>(index) +                           static_cast<double>(grid.strides[i]);
        ...
        result[i] = ((-0.5 * dl[i] * dl[i] + dl[i] - 0.5) * p1                      +  (1.5 * dl[i] * dl[i] - 2 * dl[i]) * p2                      + (-1.5 * dl[i] * dl[i] + dl[i] + 0.5) * p3                      + (0.5 * dl[i] * dl[i]) * p4);
...
struct BSplineValue
{
                                 ...
                                 const agsis::vect<double, 4> c[D])
    {
        ...
        for (int i = -1; i < 3;)
        {
            value += BSplineValue<N + 1, T, D, RC>::compute(index + grid.strides[N]                                                            * clamp(0, grid.shape[N] - 1,
