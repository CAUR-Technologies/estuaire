...
def generate(env):
    env.CGInverse =  new.instancemethod(CGInverse, env, env.__class__)
    env.CGInverse2 =  new.instancemethod(CGInverse2, env, env.__class__)

    env['BUILDERS']['CGInverseProblem'] =\
            Builder(action = Action(CGAction,
                                    strfunction = logger.default_strfun("Conjugate Gradient")))
