...

{
    ...
    do
    {
        ...
        tt += dl / read_cubic_bspline(velocity, last + 0.5 * dt);
            //cerr << cpos << n << endl;
                    ...
                    double h_init = 1)
{
    ...
    do
    {
        ...
        tt += integrate_segment(indices, sensivity, it, velocity, last + 0.5 * dt, dl) ;
            //cerr << cpos << n << endl;
...
struct IntegrateSegment
{
                                 ...
                                 size_t &it)
    {
        ...
        for (int i = -1; i < 3;)
        {
            next_index = index + grid.strides[N] *                          clamp(0, grid.shape[N] - 1, static_cast<int>(fv[N]) + i);
...
// Cython Goodness
template <typename T, std::size_t  D, bool BC>
double sensivity_RK2(const OrthogonalGrid<T, D, BC> &arrival,
                          const ArrayDescriptor<T, D, BC> &velocity,
                          const agsis::vect<double, D> &start,
                          const agsis::vect<double, D> &finish,
                          ArrayDescriptor<size_t, 1, BC> &indices,
...
template <typename T, std::size_t  D, bool BC>
double raytrace_RK2(const OrthogonalGrid<T, D, BC> &arrival,
                       const ArrayDescriptor<T, D, BC> &velocity,
                       const agsis::vect<double, D> &start,
                       const agsis::vect<double, D> &finish,
                       ArrayDescriptor<agsis::vect<double, D>, 1, BC> &path,
...
template <typename T, std::size_t  D, bool BC>
double traveltime_RK2(const OrthogonalGrid<T, D, BC> &arrival,
                       const ArrayDescriptor<T, D, BC> &velocity,
                       const agsis::vect<double, D> &start,
                       const agsis::vect<double, D> &finish,
                       const double h = 1)
...
template <typename T, std::size_t D, bool BC>
double c_ray_sensivity2(const OrthogonalGrid<T, D, BC> &velocity,
                           const ArrayDescriptor<agsis::vect<double, D> , 1, BC> &raypath,
                           ArrayDescriptor<size_t, 1, BC> &indices,
                           ArrayDescriptor<double, 1, BC> &sensivity)
{
...
template <typename T, std::size_t D, bool BC>
double c_ray_traveltime2(const OrthogonalGrid<T, D, BC> &velocity,
                             const ArrayDescriptor<agsis::vect<double, D> , 1, BC> &raypath)
{
    return c_ray_traveltime(velocity, raypath);
}
template <typename T, std::size_t  D, bool BC>
double sensivity_RK3(const OrthogonalGrid<T, D, BC> &arrival,
                          const ArrayDescriptor<T, D, BC> &velocity,
                          const agsis::vect<double, D> &start,
                          const agsis::vect<double, D> &finish,
                          ArrayDescriptor<size_t, 1, BC> &indices,
...
template <typename T, std::size_t  D, bool BC>
double raytrace_RK3(const OrthogonalGrid<T, D, BC> &arrival,
                       const ArrayDescriptor<T, D, BC> &velocity,
                       const agsis::vect<double, D> &start,
                       const agsis::vect<double, D> &finish,
                       ArrayDescriptor<agsis::vect<double, D>, 1, BC> &path,
...
template <typename T, std::size_t  D, bool BC>
double traveltime_RK3(const OrthogonalGrid<T, D, BC> &arrival,
                       const ArrayDescriptor<T, D, BC> &velocity,
                       const agsis::vect<double, D> &start,
                       const agsis::vect<double, D> &finish,
                       const double h = 1)
...
template <typename T, std::size_t D, bool BC>
double c_ray_sensivity3(const OrthogonalGrid<T, D, BC> &velocity,
                           const ArrayDescriptor<agsis::vect<double, D> , 1, BC> &raypath,
                           ArrayDescriptor<size_t, 1, BC> &indices,
                           ArrayDescriptor<double, 1, BC> &sensivity)
{
...
template <typename T, std::size_t D, bool BC>
double c_ray_traveltime3(const OrthogonalGrid<T, D, BC> &velocity,
                             const ArrayDescriptor<agsis::vect<double, D> , 1, BC> &raypath)
{
    return c_ray_traveltime(velocity, raypath);
}
