...
def add_station(db, stkey):
    cursor = db.execute("SELECT id FROM station WHERE X = :X AND Y = :Y AND Z = :Z", stkey )
    staid = cursor.fetchone()
    if staid is None:
        db.execute("INSERT INTO station(X, Y, Z) VALUES(:X,:Y,:Z)", stkey)
